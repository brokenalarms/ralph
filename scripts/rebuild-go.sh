#!/usr/bin/env bash
# Rebuild Go ralph binary if go/ or prompts/ files changed between $1 and $2 (or HEAD).
set -euo pipefail
...
if git diff-tree --no-commit-id --name-only -r "$old" "$new" | grep -qE '^(go|prompts)/'; then
  # Copy prompts into go/cmd/ralph/ for embedding
