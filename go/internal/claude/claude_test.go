...
// Verifies that Run() sends the initial prompt via stdin as a stream-json
// message when no CmdFactory is set. This is the core of the stdin injection
// feature: --input-format stream-json replaces -p, and the prompt arrives as
// a UserInputMessage on stdin.
func TestRun_SendsPromptViaStdin(t *testing.T) {
	dir := t.TempDir()
	rawLog := filepath.Join(dir, "raw.log")
	signals := DefaultSignalPaths(dir)
	stdinCapture := filepath.Join(dir, "stdin.txt")

	// Create a fake "claude" that reads one line from stdin, writes it to a
	// file, then writes a completion signal and exits.
...
echo "done" > %s
sleep 0.5
`, stdinCapture, signals.Complete)
	os.WriteFile(fakeClaude, []byte(script), 0o755)

	// Override PATH so Run() finds our fake claude.
	...
	// Verify the prompt was sent as a stream-json UserInputMessage.
	data, err := os.ReadFile(stdinCapture)
	if err != nil {
		t.Fatalf("failed to read stdin capture: %v", err)
	}
	got := strings.TrimSpace(string(data))
	want := `{"type":"user_input_text","content":"build the feature"}`
	if got != want {
		t.Errorf("stdin content = %q, want %q", got, want)
	}

	// Verify the stdin pipe was cleaned up.
	runner.mu.Lock()
	defer runner.mu.Unlock()
	if runner.stdinPipe != nil {
		t.Error("stdinPipe should be nil after Run() returns")
	}
}

// Verifies that Run() with CmdFactory does NOT set up stdin pipe,
// preventing test stubs from blocking on cmd.Wait().
func TestRun_CmdFactory_NoStdinPipe(t *testing.T) {
	dir := t.TempDir()
	rawLog := filepath.Join(dir, "raw.log")
	signals := DefaultSignalPaths(dir)
	...
	// Write signal so the run completes quickly.
	go func() {
		time.Sleep(200 * time.Millisecond)
		tmp := signals.Complete + ".tmp"
		os.WriteFile(tmp, []byte("done"), 0o644)
	...
	// CmdFactory path should not create stdin pipe.
	runner.mu.Lock()
	defer runner.mu.Unlock()
	if runner.stdinPipe != nil {
		t.Error("stdinPipe should be nil when CmdFactory is used")
	}
}

// Verifies that feedback injection works during a live Run() with stdin pipe,
// proving the full end-to-end flow: process receives prompt via stdin, then
// receives feedback as a follow-up message on the same pipe.
func TestRun_FeedbackInjectedDuringLiveRun(t *testing.T) {
	dir := t.TempDir()
	rawLog := filepath.Join(dir, "raw.log")
	signals := DefaultSignalPaths(dir)
	feedbackFile := filepath.Join(dir, "feedback")
	...
	// Fake claude: reads stdin line-by-line, appending each to a file.
	// Stays alive until killed. Uses a read loop so lines flush immediately.
	fakeClaude := filepath.Join(dir, "claude")
	script := fmt.Sprintf(`#!/bin/sh
while IFS= read -r line; do
  echo "$line" >> %s
done
# Keep running after stdin EOF until killed.
while true; do sleep 0.1; done
`, stdinCapture)
	os.WriteFile(fakeClaude, []byte(script), 0o755)
	t.Setenv("PATH", dir+":"+os.Getenv("PATH"))

	// Write feedback after a delay, then signal completion.
	go func() {
		time.Sleep(500 * time.Millisecond)
		os.WriteFile(feedbackFile, []byte("fix the tests please"), 0o644)
		time.Sleep(500 * time.Millisecond)
		tmp := signals.Complete + ".tmp"
		os.WriteFile(tmp, []byte("done"), 0o644)
		os.Rename(tmp, signals.Complete)
	...
	if result.FeedbackKill {
		t.Error("feedback should be injected via stdin, not kill the agent")
	}
	if !result.SignalDetected {
		t.Error("expected signal detection")
	}

	// Verify stdin captured both the initial prompt and the feedback.
	data, err := os.ReadFile(stdinCapture)
	if err != nil {
		t.Fatalf("failed to read stdin capture: %v", err)
	}
	...
	if !strings.Contains(content, "do the task") {
		t.Errorf("stdin should contain initial prompt, got: %q", content)
	}
	if !strings.Contains(content, "fix the tests please") {
		t.Errorf("stdin should contain feedback, got: %q", content)
	}
}
