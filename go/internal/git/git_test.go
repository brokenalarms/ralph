...
// ResetToStackHead fetches a branch from origin and hard-resets the worktree
// to it, setting PrevBranch so the next PR targets the stack head.
func TestResetToStackHead_StacksOnPreviousBranch(t *testing.T) {
	project, _ := initBareRepo(t)
	ralphDir := filepath.Join(project, ".ralph")
	st := newMemState()
	log := &testLog{}
	...
	// Create a branch with a commit, push it to origin.
	mgr.RenameBranchForTask("task A", "")
	writeFile(t, mgr.WorkDir, "a.txt", "task A work\n")
	run(t, "git", "-C", mgr.WorkDir, "commit", "-m", "task A")
	taskABranch := mgr.WorktreeBranch
	run(t, "git", "-C", mgr.WorkDir, "push", "-u", "origin", taskABranch)
	taskARev := mgr.HeadRev()

	// Switch back to a fresh branch for task B.
	mgr.PrepareForNextTask()

	// Reset to the stack head (task A's branch).
	ok := mgr.ResetToStackHead(context.Background(), taskABranch)
	if !ok {
		t.Fatal("ResetToStackHead returned false, expected true")
	}

	if mgr.HeadRev() != taskARev {
		t.Errorf("HEAD = %s, want %s (task A tip)", mgr.HeadRev(), taskARev)
	}
	if mgr.PrevBranch != taskABranch {
		t.Errorf("PrevBranch = %q, want %q", mgr.PrevBranch, taskABranch)
	}
	prevFromState, _ := st.Read("prev_branch")
	if prevFromState != taskABranch {
		t.Errorf("state prev_branch = %q, want %q", prevFromState, taskABranch)
	}
}

// ResetToStackHead returns false when the branch doesn't exist on origin,
// allowing the caller to fall back to the default branch.
func TestResetToStackHead_FallsBackWhenBranchMissing(t *testing.T) {
	project, _ := initBareRepo(t)
	ralphDir := filepath.Join(project, ".ralph")
	...
	originalRev := mgr.HeadRev()
	ok := mgr.ResetToStackHead(context.Background(), "nonexistent-branch")
	if ok {
		t.Error("ResetToStackHead should return false for missing branch")
	}
	if mgr.HeadRev() != originalRev {
		t.Errorf("HEAD should not change on failure, got %s want %s", mgr.HeadRev(), originalRev)
	}
}
