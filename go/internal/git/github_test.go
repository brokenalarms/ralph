...
// neutralizeCISkipMarkers replaces each recognized GitHub skip-CI bracket
// marker with a hyphenated form so bead text describing CI mechanics cannot
// inadvertently suppress workflows on merge.
func TestNeutralizeCISkipMarkers(t *testing.T) {
	cases := []struct {
		input string
		want  string
	}{
		...
		{"[SKIP CI]", "[SKIP-CI]"},
		{"[Skip Ci]", "[Skip-Ci]"},
		{"text before [skip ci] text after", "text before [skip-ci] text after"},
		{"no marker here", "no marker here"},
		{"multiple [skip ci] and [ci skip]", "multiple [skip-ci] and [ci-skip]"},
	}
	for _, tc := range cases {
		got := neutralizeCISkipMarkers(tc.input)
		if got != tc.want {
			t.Errorf("neutralizeCISkipMarkers(%q) = %q, want %q", tc.input, got, tc.want)
		}
	}
}

// formatPRBody neutralizes CI-skip markers in bead-derived content so a bead
// describing CI skip behavior cannot inadvertently suppress GitHub workflows on
// the resulting squash-merge commit.
func TestFormatPRBody_NeutralizesCISkipMarkers(t *testing.T) {
	// skipRe matches the literal bracket markers GitHub recognises.
	skipRe := regexp.MustCompile(`(?i)\[(skip ci|ci skip|no ci|skip actions|actions skip)\]`)
	...
	for _, marker := range markers {
		body := formatPRBody(
			"This PR uses "+marker+" to explain CI",
			"Criterion: "+marker+" is described",
			"Summary mentions "+marker+" for context",
		)
		if skipRe.MatchString(body) {
			t.Errorf("formatPRBody did not neutralize CI-skip marker %q; body still triggers GitHub skip: %q", marker, body)
		}
	}
}
