...
func (m *Manager) PushAndCreatePR(ctx context.Context, taskDesc string) error {
	...
	prCmd := exec.CommandContext(ctx, "gh", "pr", "list", "--head", m.WorktreeBranch, "--state", "open",
...
func (m *Manager) AutoMergeCurrentBranch(ctx context.Context) (bool, error) {
	...
	cmd := exec.CommandContext(ctx, "gh", "pr", "list", "--head", m.WorktreeBranch, "--state", "open",
		"--json", "number", "--jq", ".[0].number", "-R", repoURL)
	out, err := cmd.Output()
	...
	mergeCmd := exec.CommandContext(ctx, "gh", m.ghMergeArgs(prNumber, repoURL)...)
	...
	if isCIGatedError(mergeOutput) {
		...
		if status == CIPassed {
			...
			retryCmd := exec.CommandContext(ctx, "gh", m.ghMergeArgs(prNumber, repoURL)...)
