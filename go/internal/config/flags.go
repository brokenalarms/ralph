...
var Flags = []FlagDef{
	...
	{
		Long: "--evolve",
		Help: "Re-exec ralph between iterations to incorporate the latest version. Use with --post-task to rebuild before re-exec.",
		Kind: KindBool,
		Apply: func(cfg *Config, _ string) error {
			cfg.Evolve = true
			return nil
	...
	},
	{
		...
		Help:      "Run a command after each task completes, before evolve re-exec. Receives RALPH_TASK_ID, RALPH_PR_NUMBER, and RALPH_MERGED env vars.",
		ConfigKey: "post_task",
		Kind:      KindString, TrackCLI: true,
		Apply: func(cfg *Config, val string) error {
			cfg.PostTask = val
