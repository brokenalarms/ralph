...
// Proves: Export runs bd export and writes JSONL to specs/beads-export.jsonl.
func TestBD_Export_WritesJSONL(t *testing.T) {
	exportData := `{"id":"t1","title":"First"}\n{"id":"t2","title":"Second"}`
	runner := func(_ context.Context, dir string, args ...string) (string, error) {
	...
	got, err := os.ReadFile(filepath.Join(b.ProjectDir, "specs", "beads-export.jsonl"))
	if err != nil {
		t.Fatalf("expected specs/beads-export.jsonl to exist: %v", err)
	}
	if string(got) != exportData+"\n" {
		t.Errorf("export content = %q, want %q", string(got), exportData+"\n")
	}
}

// Proves: Export creates the specs/ directory if it doesn't exist.
func TestBD_Export_CreatesSpecsDir(t *testing.T) {
	runner := func(_ context.Context, dir string, args ...string) (string, error) {
		if len(args) > 0 && args[0] == "export" {
			return `{"id":"t1"}`, nil
		}
	...
	if _, err := os.Stat(specsDir); !os.IsNotExist(err) {
		t.Fatal("specs/ should not exist before Export")
	}
	if err := b.Export(); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(specsDir); err != nil {
		t.Error("expected specs/ directory to be created by Export")
	}
}

// Proves: CloseTask triggers an export after closing.
func TestBD_CloseTask_TriggersExport(t *testing.T) {
	...
	if !exportCalled {
		t.Error("expected Export to be called after CloseTask")
	}
}

// Proves: SkipTask triggers an export after skipping.
func TestBD_SkipTask_TriggersExport(t *testing.T) {
	var exportCalled bool
	runner := func(_ context.Context, dir string, args ...string) (string, error) {
		if len(args) > 0 && args[0] == "export" {
			exportCalled = true
	...
	b := setupBD(t, runner)
	if err := b.SkipTask("abc123", "stuck"); err != nil {
		t.Fatal(err)
	}
	if !exportCalled {
		t.Error("expected Export to be called after SkipTask")
	}
}

// Proves: SetState triggers an export after state change.
func TestBD_SetState_TriggersExport(t *testing.T) {
	var exportCalled bool
	runner := func(_ context.Context, dir string, args ...string) (string, error) {
		if len(args) > 0 && args[0] == "export" {
			exportCalled = true
	...
	b := setupBD(t, runner)
	if err := b.SetState("abc123", "phase", "implementing", ""); err != nil {
		t.Fatal(err)
	}
	if !exportCalled {
		t.Error("expected Export to be called after SetState")
	}
}

// Proves: ReopenTask triggers an export after reopening.
func TestBD_ReopenTask_TriggersExport(t *testing.T) {
	var exportCalled bool
	runner := func(_ context.Context, dir string, args ...string) (string, error) {
		if len(args) > 0 && args[0] == "export" {
			exportCalled = true
	...
	b := setupBD(t, runner)
	if err := b.ReopenTask("abc123"); err != nil {
		t.Fatal(err)
	}
	if !exportCalled {
		t.Error("expected Export to be called after ReopenTask")
	}
}
